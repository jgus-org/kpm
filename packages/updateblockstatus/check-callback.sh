...
SCRIPTLET=${FIXTURE_DIRECTORY}/mnt/us/documents/updateblock.sh

trap 'rm -rf "${FIXTURE_DIRECTORY}"' EXIT

mkdir -p "${MOCK_DIRECTORY}" "${FIXTURE_DIRECTORY}/package/scriptlets" "${FIXTURE_DIRECTORY}/mnt/us/documents"
printf '%s\n' 'exec /var/local/kmc/bin/kpm launch updateblockstatus' >"${FIXTURE_DIRECTORY}/package/scriptlets/updateblock.sh"
cp "${FIXTURE_DIRECTORY}/package/scriptlets/updateblock.sh" "${SCRIPTLET}"
...
test "$(cat "${SCRIPTLET}")" = 'exec /var/local/kmc/bin/kpm launch updateblockstatus'
printf '%s\n' 'exec /var/local/kmc/bin/kpm launch updateblockstatus' >"${FIXTURE_DIRECTORY}/foreign.sh"
rm "${SCRIPTLET}"
